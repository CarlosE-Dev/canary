function endTaskModalWindow(player, storage)
	...
	local playerTaskKills = player:getTaskKills(data.storagecount)
	local numberOfTasks = 0
	local extraForNext = 0

	if playerTaskKills > 0 then
		if playerTaskKills >= data.total and playerTaskKills < data.total * 2 then
			numberOfTasks = 1
			extraForNext = playerTaskKills - data.total
		end

		if playerTaskKills > data.total and playerTaskKills >= data.total * 2 and playerTaskKills < data.total * 3 then
			numberOfTasks = 2
			extraForNext = playerTaskKills - data.total * 2
		end

		if playerTaskKills > data.total and playerTaskKills > data.total * 2 and playerTaskKills >= data.total * 3 then
			numberOfTasks = 3
			extraForNext = playerTaskKills - data.total * 3
		end
	end

	if extraForNext <= 0 then
		extraForNext = 0
	end

	player:setStorageValue(data.storagecount + 1000, extraForNext)

	local newmessage
	local completion = false
	if playerTaskKills < data.total then
		...
		if player:getStorageValue(taskOptions.bonusReward) >= 1 then
			...
			for _, info in pairs(data.rewards) do
				if info[1] == "item" then
					...
					if chance <= info[2].chance then
						window:addChoice("- " .. info[2] .. " " .. ItemType(info[2].id):getName())
						player:addItem(info[2].id, info[2].count)
					end
				elseif info[1] == "exp" then
					local baseRate = player:getFinalBaseRateExperience()
					player:addExperience(info[2] * taskOptions.bonusRate * baseRate)
			...
			for _, info in pairs(data.rewards) do
				if info[1] == "item" then
					if numberOfTasks > 1 then
						for i = 1, numberOfTasks do
							local chance = math.random(1, 1000)
							if chance <= info[2].chance then
								window:addChoice("Item: " .. info[2].count .. " " .. ItemType(info[2].id):getName())
								player:addItem(info[2].id, info[2].count)
							end
						end
					else
				...
				elseif info[1] == "exp" then
					local baseRate = player:getFinalBaseRateExperience()
					player:addExperience(info[2] * numberOfTasks * baseRate)
					player:getPosition():sendMagicEffect(CONST_ME_PRISMATIC_SPARK)
					player:say("Exp: " .. info[2] * numberOfTasks * baseRate .. "", TALKTYPE_MONSTER_SAY)
					if taskOptions.selectLanguage == 1 then
						window:addChoice(task_pt_br.choiceText .. "" .. info[2])
					else
						window:addChoice("Experience: " .. info[2] * numberOfTasks * baseRate)
					end
				elseif tonumber(info[1]) then
					window:addChoice("- " .. info[2] * numberOfTasks .. " " .. ItemType(info[1]):getName())
					player:addItem(info[1], info[2] * numberOfTasks)
					player:setStorageValue(taskOptions.uniqueTaskStorage, -1)
					player:getPosition():sendSingleSoundEffect(SOUND_EFFECT_TYPE_ACTION_LEVEL_ACHIEVEMENT, player:isInGhostMode() and nil or player)
					player:getPosition():sendMagicEffect(CONST_ME_PRISMATIC_SPARK)
					if taskOptions.selectLanguage == 1 then
